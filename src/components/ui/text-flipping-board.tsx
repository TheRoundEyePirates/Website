...
// Vintage gold tile used for the occasional scramble flash.
const ACCENT_GOLD: AccentColor = {
  top: "bg-[#ecd9a8]",
  bottom: "bg-[#e4cf9b]",
  text: "text-ink",
};
...
const FlapCell = React.memo(function FlapCell({
  ...
  return (
    <div className="flex aspect-3/6 flex-col overflow-hidden rounded-[2px] border border-ink/20 md:rounded-[3px] md:border-2">
      {/* Flap content area */}
      <div className="relative flex-1 perspective-dramatic transform-3d">
        <div className="absolute inset-0 z-40 hidden flex-row items-center justify-center md:flex">
          <div className="h-1/2 w-px rounded-tr-sm rounded-br-sm bg-ink/15" />
          <div className="flex h-px flex-1 bg-ink/15" />
          <div className="h-1/2 w-px rounded-tl-sm rounded-bl-sm bg-ink/15" />
        </div>
        ...
        {/* Split line */}
        <div className="pointer-events-none absolute inset-x-0 top-1/2 z-20 h-px -translate-y-[0.5px] bg-gold/40" />
      </div>

      {/* Bottom stripes – decorative hinge, outside the flap area */}
      <div
        className="h-2 w-full bg-[repeating-linear-gradient(to_bottom,currentColor_0,currentColor_1px,transparent_1px,transparent_0.15rem)] text-gold opacity-25 md:h-4 md:bg-[repeating-linear-gradient(to_bottom,currentColor_0,currentColor_1px,transparent_1px,transparent_0.2rem)]"
        style={{
...
const ColorCell = React.memo(function ColorCell({ color }: { color: string }) {
  return (
    <div
      className="aspect-3/5 rounded-[3px] border-2 border-ink/20"
...
export function TextFlippingBoard({
  ...
  return (
    <div
      className={cn(
        "relative mx-auto w-full max-w-3xl rounded-xl border border-ink/15 bg-sand-deep/70 p-2 shadow-[0_10px_30px_-12px_rgba(0,0,0,0.6)] md:rounded-2xl md:p-4",
        className,
      )}
    >
      <div
